...
materialized cash, asset, cost basis, position, realized/unrealized PnL, equity,
fees, slippage and closed-trade projection share one `BEGIN IMMEDIATE` transaction.
Stored history is replayed through the accepted P2 `PortfolioLedger` before every
write; no alternate accounting model exists. Duplicate, insufficient-cash,
position mismatch, changed spec, invalid mark, tamper and injected failures roll
back or fail before mutation. `paper-portfolio-check` committed two fills, rejected
duplicate replay, reopened deterministic evidence and finished `FLAT` with cash
